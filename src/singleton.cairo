...
mod Singleton {
    ...
    impl InternalFunctions of InternalFunctionsTrait {
        ...
        fn update_position(
            ...
            if collateral_delta.is_non_zero() && context.position.nominal_debt != 0 {
    ...
    impl SingletonImpl of super::ISingleton<ContractState> {
        ...
        fn transfer_position(ref self: ContractState, params: TransferPositionParams) {
            ...
            let collateral_asset_fee_shares = from_context.collateral_asset_fee_shares;
            let debt_asset_fee_shares = from_context.debt_asset_fee_shares;
            ...
            assert!(
                (collateral_delta.is_negative || collateral_delta.abs == 0)
                    && (debt_delta.is_negative || debt_delta.abs == 0),
            ...
            // reload context since 'from' and 'to' context might referencing be the same storage vars
            let mut from_context = self.context(pool_id, from_collateral_asset, from_debt_asset, from_user);
            // fee shares have to be re-attributed to the reloaded `from` context since the rate accumulator has
            // already been updated (written to storage)
            from_context.collateral_asset_fee_shares = collateral_asset_fee_shares;
            from_context.debt_asset_fee_shares = debt_asset_fee_shares;

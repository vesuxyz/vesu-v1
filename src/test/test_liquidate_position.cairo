...
mod TestLiquidatePosition {
    ...
    //     extension
    //         .set_asset_parameter(pool_id, collateral_asset.contract_address, 'liquidation_discount', 90 * SCALE / 100);
    ...
    fn test_liquidate_position_scenario_1_full_liquidation() {
        ...
        let liquidation_discount = 90 * SCALE / 100;
        let min_collateral_to_receive = collateral;
        let debt_to_repay = (((collateral * collateral_price / SCALE) * liquidation_discount) / debt_price)
            * debt_scale
            / collateral_scale;
        ...
        extension
            .set_liquidation_config(
                ...
                LiquidationConfig { liquidation_discount: liquidation_discount.try_into().unwrap() }
            );
        stop_prank(CheatTarget::One(extension.contract_address));
    ...
    fn test_liquidate_position_scenario_5_partial_liquidation() {
        ...
        let debt_to_repay = (((collateral * collateral_price / collateral_scale) * liquidation_discount / SCALE)
            * debt_scale)
            / (debt_price * 2);

...
mod TestOracle {
    ...
    use vesu::{
        ...
        singleton::{
            ISingletonDispatcherTrait, ISingletonDispatcher, Amount, AmountType, AmountDenomination,
            ModifyPositionParams,
        },
        test::{
            setup::{setup, setup_env, TestConfig, LendingTerms},
        ...
        extension::{
            ...
            default_extension::{
                IDefaultExtensionDispatcherTrait, IDefaultExtensionDispatcher, InterestRateModel, PragmaOracleParams,
    ...
    fn create_custom_pool(
        ...
        let collateral_asset_oracle_params = PragmaOracleParams {
            pragma_key: 19514442401534788, timeout, number_of_sources
        };
        let debt_asset_oracle_params = PragmaOracleParams {
            pragma_key: 5500394072219931460, timeout, number_of_sources
        };
        ...
        let models = array![interest_rate_model, interest_rate_model].span();
        ...
        extension
            .create_pool(
                asset_params, max_position_ltv_params, models, oracle_params, liquidation_params, shutdown_params
            );
    ...
    fn test_get_price_high() {
        ...
        pragma_oracle.set_price(19514442401534788, max);
        // set debt asset price
        pragma_oracle.set_price(5500394072219931460, max);
    ...
    fn test_is_valid_timeout() {
        ...
        pragma_oracle.set_last_updated_timestamp(19514442401534788, get_block_timestamp());
    ...
    #[test]
    fn test_is_valid_timeout_stale_rpice() {
        let (singleton, extension, config, users) = setup_env(
            Zeroable::zero(), Zeroable::zero(), Zeroable::zero(), Zeroable::zero(),
        );
        let TestConfig{collateral_asset, debt_asset, .. } = config;
    ...
    fn test_is_valid_sources_reached() {
        ...
        pragma_oracle.set_num_sources_aggregated(19514442401534788, min_number_of_sources + 1);

use snforge_std::{declare, ContractClassTrait, start_prank, stop_prank, start_warp, stop_warp, CheatTarget};
...
use vesu::{
    ...
    extension::default_extension::{
        IDefaultExtensionDispatcher, IDefaultExtensionDispatcherTrait, InterestRateModel, PragmaOracleParams,
...
fn deploy_with_args(name: felt252, constructor_args: Array<felt252>) -> ContractAddress {
...
fn setup_env(
    ...
    // create pool config
    let pool_id = singleton.calculate_pool_id(extension.contract_address, 0);
...
fn test_interest_rate_model() -> InterestRateModel {
    InterestRateModel {
...
fn create_pool(
    ...
    let interest_rate_model = model.unwrap_or(test_interest_rate_model());
    ...
    let max_position_ltv_params_2 = LTVParams { collateral_asset_index: 0, debt_asset_index: 2, ltv: 85 * PERCENT };
    ...
    let shutdown_ltv_params_0 = LTVParams { collateral_asset_index: 1, debt_asset_index: 0, ltv: 75 * PERCENT };
    let shutdown_ltv_params_1 = LTVParams { collateral_asset_index: 0, debt_asset_index: 1, ltv: 75 * PERCENT };
    let shutdown_ltv_params_2 = LTVParams { collateral_asset_index: 0, debt_asset_index: 2, ltv: 75 * PERCENT };
    let shutdown_ltv_params = array![shutdown_ltv_params_0, shutdown_ltv_params_1, shutdown_ltv_params_2].span();
    ...
    let max_position_ltv_params = array![
        max_position_ltv_params_0, max_position_ltv_params_1, max_position_ltv_params_2
    ]
        .span();
    let models = array![interest_rate_model, interest_rate_model, interest_rate_model].span();
    ...
    extension
        .create_pool(asset_params, max_position_ltv_params, models, oracle_params, liquidation_params, shutdown_params);
...
fn setup_pool(
    ...
    create_pool(extension, config, users.creator, model);
    ...
    let rate_accumulator = singleton.asset_configs(pool_id, debt_asset.contract_address).last_rate_accumulator;
    let nominal_debt_to_draw = singleton.calculate_nominal_debt(debt_to_draw, rate_accumulator, debt_scale);

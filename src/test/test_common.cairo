...
mod TestCommon {
    ...
    fn test_debt_inverse_relation(debt: u128) {
        let asset_scale = 100_000_000;
        let initial_debt = debt.into() * asset_scale;
        let rate_accumulator = SCALE;
        let nominal_debt = calculate_nominal_debt(initial_debt, rate_accumulator, asset_scale);
        let debt_calc = calculate_debt(nominal_debt, rate_accumulator, asset_scale);
        assert!(debt_calc == initial_debt, "Debt calculations aren't invertible");
    }

    // checks rounding occurs
    #[test]
    #[fuzzer(runs: 256, seed: 101)]
    fn test_nominal_debt_rounding(mut init_debt: u128, mut rate_accumulator: u8) {
        ...
        let nominal_debt = calculate_nominal_debt(initial_debt, rate_accumulator, SCALE);
    ...
    #[test]
    fn test_nominal_debt_precision() {
        let nominal_debt_rounded = calculate_nominal_debt(22_222_222, 3 * SCALE, 100_000_000);
        assert!(nominal_debt_rounded == 74074073333333334, "Debt is rounding is not precise");
        let nominal_debt_rounded = calculate_nominal_debt(55_555_555, 3 * SCALE, 100_000_000);
        assert!(nominal_debt_rounded == 185185183333333334, "Debt is rounding is not precise");
        let nominal_debt_rounded = calculate_nominal_debt(77_777_777, 3 * SCALE, 100_000_000);
        assert!(nominal_debt_rounded == 259259256666666667, "Debt is rounding is not precise");
        let nominal_debt_rounded = calculate_nominal_debt(88_888_888, 3 * SCALE, 100_000_000);
        assert!(nominal_debt_rounded == 296296293333333334, "Debt is rounding is not precise");
        let nominal_debt_rounded = calculate_nominal_debt(77_777_777, 2 * SCALE, 100_000_000);
        assert!(nominal_debt_rounded == 388888885000000000, "Debt is rounding is not precise");
    }

    #[test]
    #[should_panic(expected: ('zero-rate-accumulator',))]
    fn test_debt_div_zero() {
        let initial_debt = 20 * SCALE;
        let rate_accumulator = 0_u256;
        calculate_nominal_debt(initial_debt, rate_accumulator, SCALE);
    }
    ...
    fn test_is_collateralized(debt_value: u128, collateral_value: u128, ratio: u8) {
        ...
        if ((collateral_value * ratio.into()) >= debt_value) {
    ...
    fn test_collateral_inverse_relation(num: u16) {
        ...
        let collateral_shares = calculate_collateral_shares(collateral_amount, config);

        let calculated_collateral = calculate_collateral(collateral_shares, config);

        assert!(calculated_collateral == collateral_amount, "Debt calculations aren't inverse");
    }
    ...
    fn test_calculate_collateral_and_debt_value(debt: u16, collateral: u16) {
        ...
        let position = Position {
            collateral_shares: calculate_collateral_shares(collateral_amount, config),
            nominal_debt: calculate_nominal_debt(initial_debt, rate_accumulator, asset_scale),
        };
        ...
        let expected_collateral = calculate_collateral(position.collateral_shares, context.collateral_asset_config);
        let expected_debt = calculate_debt(position.nominal_debt, config.last_rate_accumulator, config.scale);
    ...
    fn test_deconstruct_collateral_shares_asset_delta() {
        ...
        let position = Position {
            collateral_shares: calculate_collateral_shares(collateral, asset_config),
            nominal_debt: calculate_nominal_debt(initial_debt, SCALE, asset_scale),
        };
        ...
        let expected_collateral_shares_delta = calculate_collateral_shares(collateral_delta.abs, asset_config);
        ...
        let original_collateral = calculate_collateral(collateral_shares_delta.abs, asset_config);
        assert!(original_collateral == collateral_amount_asset_delta.value.abs, "Retrieve collateral failed");
    }
    ...
    fn test_deconstruct_native_collateral_delta() {
        ...
        assert!(
            collateral_delta == i257_new(
                calculate_collateral(collateral_shares_delta.abs, asset_config), collateral_shares_delta.is_negative
            ),
            "Deconstruct collateral failed"
        );
    ...
    fn test_deconstruct_native_collateral_target() {
        ...
        let expected_delta = calculate_collateral(
            position.collateral_shares - collateral_amount_native_target.value.abs, asset_config
        );
        ...
        let expected_delta = calculate_collateral(
            collateral_amount_native_target.value.abs - position.collateral_shares, asset_config
        );
    ...
    fn test_deconstruct_asset_collateral_target() {
        ...
        let expected_shares_delta = calculate_collateral_shares(
            collateral_amount_asset_target.value.abs - position_collateral, asset_config
        );
        ...
        let expected_shares_delta = calculate_collateral_shares(
            position_collateral - collateral_amount_asset_target.value.abs, asset_config
        );
    ...
    fn test_deconstruct_native_debt_delta() {
        ...
        assert!(
            debt_delta == i257_new(
                calculate_debt(nominal_debt_delta.abs, rate_accumulator, asset_scale), nominal_debt_delta.is_negative
            ),
    ...
    fn test_deconstruct_asset_debt_delta() {
        ...
        assert!(
            nominal_debt_delta == i257_new(
                calculate_nominal_debt(debt_delta.abs, rate_accumulator, asset_scale), debt_delta.is_negative
            ),
            "Deconstruct nominal debt failed"
        );
    }
    ...
    fn test_deconstruct_native_debt_target() {
        ...
        let expected_debt_delta = calculate_debt(
            position.nominal_debt - debt_amount_native_target.value.abs, rate_accumulator, asset_scale
        );
    ...
    fn test_deconstruct_asset_debt_target() {
        ...
        let expected_nominal_delta = calculate_nominal_debt(
            position_debt - debt_amount_asset_target.value.abs, rate_accumulator, asset_scale
        );
        ...
        let expected_nominal_delta = calculate_nominal_debt(
            debt_amount_asset_target.value.abs - position_debt, rate_accumulator, asset_scale
        );

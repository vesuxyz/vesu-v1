...
mod TestCommon {
    ...
    fn test_calculate_nominial_debt_calculate_debt_inverse_relation(seed: u128) {
        ...
        let debt_calc = calculate_debt(nominal_debt, rate_accumulator, asset_scale, false);
        assert!(debt_calc == debt, "Debt calculations aren't invertible");
    }

    #[test]
    #[fuzzer(runs: 256, seed: 100)]
    fn test_calculate_nominal_calculate_debt_inverse_relation_2(seed: u16) {
        let asset_scale = SCALE;
        let nominal_debt = seed.into() * SCALE + seed.into();
        let rate_accumulator = SCALE;
        let debt = calculate_debt(nominal_debt, rate_accumulator, asset_scale, false);
    ...
    fn test_is_collateralized(debt_value: u128, collateral_value: u128, ratio: u8) {
        ...
        if (ratio == 0 || (collateral_value * ratio.into()) >= debt_value) {
    ...
    fn test_collateral_inverse_relation(num: u16) {
        ...
        let collateral_shares = calculate_collateral_shares(collateral_amount, config, true);

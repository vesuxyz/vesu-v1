...
mod map_list_component {
    ...
    impl MapListTrait<TContractState, +HasComponent<TContractState>> of Trait<TContractState> {
        ...
        // ..., previous_item, item_after, ... -> ..., previous_item, item_to_add, item_after, ...
        fn insert_before(
            ref self: ComponentState<TContractState>, list_id: felt252, item_after: u64, item_to_add: u64,
        ) {
            assert!(item_to_add != 0, "cannot-insert-zero");
            assert!(item_after == 0 || self.contains(list_id, item_after), "cannot-find-item-after");
            let first = self.lists.read((list_id, 0));
            if first == 0 {
                self.lists.write((list_id, 0), item_to_add);
            } else {
                let previous_item = self.find_item_before(list_id, item_after);
                self.lists.write((list_id, previous_item), item_to_add);
                self.lists.write((list_id, item_to_add), item_after);
            }
        }

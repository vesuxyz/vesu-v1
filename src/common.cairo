...
fn calculate_nominal_debt(debt: u256, rate_accumulator: u256, asset_scale: u256, round_up: bool) -> u256 {
    ...
    if remainder == 0 || !round_up {
        safe_div(nominal_debt, asset_scale, false)
    } else {
        (safe_div(nominal_debt + 1, asset_scale, true))
    }
}
...
fn calculate_collateral_shares(collateral: u256, asset_config: AssetConfig, round_up: bool) -> u256 {
    ...
    let scaled_collateral_shares = u256 { low: scaled_collateral_shares.limb0, high: scaled_collateral_shares.limb1 };
    if remainder == 0 || !round_up {
        return safe_div(scaled_collateral_shares, SCALE, false);
    } else {
        return safe_div(scaled_collateral_shares + 1, SCALE, true);
    }
}
...
fn calculate_collateral(collateral_shares: u256, asset_config: AssetConfig, round_up: bool) -> u256 {
    ...
    let scaled_collateral = u256 { low: scaled_collateral.limb0, high: scaled_collateral.limb1 };
    if remainder == 0 || !round_up {
        return safe_div(scaled_collateral, SCALE, false);
    } else {
        return safe_div(scaled_collateral + 1, SCALE, true);
    }
}
...
/// If `max_ltv_ratio` is 0, the position is always considered collateralized.
/// # Arguments
...
fn apply_position_update_to_context(
    ...
    if collateral_delta > Zeroable::zero() {
        ...
        if collateral_shares_delta.abs > context.position.collateral_shares {
            collateral_shares_delta.abs = context.position.collateral_shares;
            collateral_delta
                .abs = calculate_collateral(collateral_shares_delta.abs, context.collateral_asset_config, false);
        }
        context.position.collateral_shares -= collateral_shares_delta.abs;
        context.collateral_asset_config.total_collateral_shares -= collateral_shares_delta.abs;
        context.collateral_asset_config.reserve -= collateral_delta.abs;
    ...
    if debt_delta > Zeroable::zero() {
        ...
        if nominal_debt_delta.abs > context.position.nominal_debt {
            nominal_debt_delta.abs = context.position.nominal_debt;
            debt_delta
                .abs =
                    calculate_debt(

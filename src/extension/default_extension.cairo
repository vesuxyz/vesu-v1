...
mod DefaultExtension {
    ...
    use vesu::extension::components::fee_model::fee_model_component::Trait;
    use vesu::{
        map_list::{map_list_component, map_list_component::MapListTrait},
        data_model::{Amount, AssetParams, AssetPrice, LTVParams, ModifyPositionParams, Context, LTVConfig},
    ...
    impl DefaultExtensionImpl of IDefaultExtension<ContractState> {
        ...
        fn create_pool(
            ...
            assert!(asset_params.len() == liquidation_params.len(), "liquidation-params-mismatch");
            ...
            while !asset_params_copy.is_empty() {
                ...
                let config = *interest_rate_configs.pop_front().unwrap();
                self.interest_rate_model.set_interest_rate_config(pool_id, asset, config);
                // set the liquidation config
                let params = *liquidation_params.pop_front().unwrap();
                let LiquidationParams { liquidation_discount } = params;
                self.position_hooks.set_liquidation_config(pool_id, asset, LiquidationConfig { liquidation_discount });
            };
        ...
        fn add_asset(
            ...
            // set the liquidation config
            let LiquidationParams { liquidation_discount } = liquidation_params;
            self.position_hooks.set_liquidation_config(pool_id, asset, LiquidationConfig { liquidation_discount });
        ...
        fn set_asset_parameter(
            ...
            assert!(get_caller_address() == self.owner.read(pool_id), "caller-not-owner");
            if parameter == 'liquidation_discount' {
                let liquidation_discount = value.try_into().unwrap();
                self.position_hooks.set_liquidation_config(pool_id, asset, LiquidationConfig { liquidation_discount });
        ...
        fn claim_fees(
            ...
            self.fee_model.claim_fees(self.singleton.read(), pool_id, collateral_asset, debt_asset);
        }
    }
        ...
        fn after_liquidate_position(
            ...
            self
                ...
                .after_liquidate_position(
                    context, collateral_delta, collateral_shares_delta, debt_delta, nominal_debt_delta, bad_debt, data
                )
        }
    }
}

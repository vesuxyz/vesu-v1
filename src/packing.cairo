...
use vesu::{data_model::{Position, AssetConfig, asset_config_exists}, math::{pow_10_or_0, log_10}, units::{PERCENT}};
...
impl AssetConfigPacking of starknet::StorePacking<AssetConfig, (felt252, felt252, felt252)> {
    fn pack(value: AssetConfig) -> (felt252, felt252, felt252) {
        ...
        let total_nominal_debt = into_u123(total_nominal_debt, 'pack-total-nomimal-debt-u123');
        ...
        let floor_decimals: u8 = log_10(value.floor);
        let scale_decimals: u8 = log_10(value.scale);
...
fn assert_storable_asset_config(asset_config: AssetConfig) {
    assert!(asset_config_exists(asset_config), "asset-config-nonexistent");

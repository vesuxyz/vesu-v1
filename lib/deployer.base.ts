...
export class BaseDeployer extends Account {
  ...
  async declareCached(name: string, payload: DeclareContractPayload) {
    if (this.alreadyDeclared[name]) {
      return this.alreadyDeclared[name];
    }
    const { class_hash } = await this.declareIfNot(payload);
  ...
  async deferContract(name: string, constructorCalldata: Calldata = []) {
    ...
    const classHash = await this.declareCached(name, payload);

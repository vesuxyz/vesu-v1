...
export class Deployer extends BaseDeployer {
  ...
  async deployEnvAndProtocol(): Promise<Protocol> {
    ...
    await this.execute([...envCalls, ...protocolCalls]);
  ...
  async deferProtocol(oracle_address: string) {
    const [singleton, calls1] = await this.deferContract("Singleton");
    const calldata = CallData.compile({ singleton: singleton.address, oracle_address });
  ...
  async setApprovals(singleton: Contract, assets: Contract[]) {
    ...
    await this.lender.execute(approvalCalls);
    await this.borrower.execute(approvalCalls);
  }
}

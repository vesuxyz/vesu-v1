...
async function profileGasUsage(transactionHash: string, provider: RpcProvider) {
  const receipt = await waitForInclusion(transactionHash, provider);
  let actualFee = 0n;
...
export function newProfiler(provider: RpcProvider, roundingMagnitude?: number) {
  ...
  return {
    async profile(
      ...
      process.stdout.write(`Profiled: ${name} ✓\n`);
      if (printProfile) {
    ...
    formatReport() {
      return Object.entries(profiles)
        .map(([name, { gasUsed }]) => {
          const roundingScale = 10 ** (roundingMagnitude ?? 2);
